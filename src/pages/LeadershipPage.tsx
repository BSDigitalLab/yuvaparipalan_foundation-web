...
import { Award, Users, MapPin, Sparkles, Crown } from 'lucide-react';
...
export const LeadershipPage: React.FC = () => {
  ...
  return (
    <>
      <SEOHead
        title="Leadership & Governance - Yuvaparipalan Foundation"
        description="Meet the founding visionaries, executive directors, distinguished patrons, and operational chiefs guiding Yuvaparipalan Foundation."
      />
                  ...
                  >
                    ...
                    <div className="text-left space-y-2 flex-1 min-w-0">
                      <div>
                        ...
                        <h3 className="font-heading font-extrabold text-lg text-slate-950 group-hover:text-emerald-900 transition-colors leading-snug mt-1.5 truncate">
                          {leader.name}
                        </h3>
                        <p className="text-xs font-extrabold text-emerald-800 leading-tight">
                          {leader.role}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
              {ourTeam.map((leader, index) => (
                <FadeIn key={leader.id} direction="up" delay={index * 0.03}>
                  ...
                  <div 
                    onClick={() => setSelectedLeader(leader)}
                    className="group cursor-pointer relative p-4 rounded-3xl bg-white hover:bg-gradient-to-b hover:from-white hover:to-emerald-50/50 border border-slate-200/80 hover:border-emerald-600/50 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between h-full overflow-hidden"
                  >
